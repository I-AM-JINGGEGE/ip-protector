package com.vpn.android.region;

public interface RegionConstants {
    String REGION_UUID_DEFAULT = "region_uuid_default";
    String REGION_CODE_DEFAULT = "default";
    String KEY_CONNECTED_VPN_IP = "KEY_CONNECTED_VPN_IP";
    String KEY_PROFILE_VPN_IP = "KEY_PROFILE_VPN_IP";
    String Key_Selected_Region_Country_Code = "key_selected_region_id";
    String KEY_AD_SWITCH = "key_ad_switch";

    String KEY_DAILY_AD_CLICKS = "ad_click_manager_daily_clicks";
    String KEY_LAST_RESET_DATE = "ad_click_manager_last_reset_date";
}
